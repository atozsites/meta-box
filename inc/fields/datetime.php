 ...
 */
class Atozsites_Datetime_Field extends Atozsites_Text_Field {
	/**
	...
	public static function admin_register_scripts() {
		...
		wp_register_style( 'Atozsites-date', Atozsites_CSS_URL . 'date.css', array( 'jquery-ui-datepicker' ), '1.8.17' );

		wp_register_style( 'jquery-ui-slider', "$url/jquery.ui.slider.css", array( 'jquery-ui-core', 'jquery-ui-theme' ), '1.8.17' );
		wp_register_style( 'jquery-ui-timepicker', "$url/jquery-ui-timepicker-addon.min.css", array( 'Atozsites-date', 'jquery-ui-slider' ), '1.5.0' );

		$url = Atozsites_JS_URL . 'jqueryui';
		...
		wp_register_script( 'Atozsites-datetime', Atozsites_JS_URL . 'datetime.js', array( 'jquery-ui-datepicker', 'jquery-ui-timepicker-i18n', 'underscore' ), Atozsites_VER, true );
		wp_register_script( 'Atozsites-date', Atozsites_JS_URL . 'date.js', array( 'jquery-ui-datepicker', 'underscore' ), Atozsites_VER, true );
		wp_register_script( 'Atozsites-time', Atozsites_JS_URL . 'time.js', array( 'jquery-ui-timepicker-i18n' ), Atozsites_VER, true );
		...
		foreach ( $handles as $handle ) {
			Atozsites_Helpers_Field::localize_script_once( "Atozsites-$handle", 'Atozsites_' . ucfirst( $handle ), $data );
		}
	}
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'Atozsites-datetime' );
	}
	...
	public static function html( $meta, $field ) {
		...
		if ( $field['timestamp'] ) {
			...
			$output .= sprintf(
				'<input type="hidden" name="%s" class="Atozsites-datetime-timestamp" value="%s">',
		...
		if ( $field['inline'] ) {
			$output .= '<div class="Atozsites-datetime-inline"></div>';
		}
	...
	public static function meta( $post_id, $saved, $field ) {
		...
		if ( $field['timestamp'] ) {
			return Atozsites_Helpers_Array::map( $meta, __CLASS__ . '::from_timestamp', $field );
		}

		if ( $field['save_format'] && $meta ) {
			return Atozsites_Helpers_Array::map( $meta, __CLASS__ . '::from_save_format', $field );
		}
