 ...
 * Map field class.
 */
class Atozsites_Map_Field extends Atozsites_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'Atozsites-map', Atozsites_CSS_URL . 'map.css', array(), Atozsites_VER );
		 ...
		 */
		$google_maps_url = apply_filters( 'Atozsites_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), Atozsites_VER, true );
		wp_enqueue_script(
			'Atozsites-map',
			Atozsites_JS_URL . 'map.js',
			array(
	...
	public static function html( $meta, $field ) {
		...
		$html    = sprintf(
			'<div class="Atozsites-map-field" data-address-field="%s">',
			esc_attr( $address )
		);

		$html .= sprintf(
			'<div class="Atozsites-map-canvas" data-default-loc="%s" data-region="%s"></div>
			<input type="hidden" name="%s" class="Atozsites-map-coordinate" value="%s">',
			esc_attr( $field['std'] ),
	...
	public static function render_map( $location, $args = array() ) {
		...
		$google_maps_url = apply_filters( 'Atozsites_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), Atozsites_VER, true );
		wp_enqueue_script( 'Atozsites-map-frontend', Atozsites_JS_URL . 'map-frontend.js', array( 'google-maps', 'jquery' ), Atozsites_VER, true );
		...
		$output = sprintf(
			'<div class="Atozsites-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>',
