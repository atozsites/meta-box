 ...
 */
class Atozsites_OSM_Field extends Atozsites_Field {
	/**
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_style( 'Atozsites-osm', Atozsites_CSS_URL . 'osm.css', array( 'leaflet' ), Atozsites_VER );
		wp_enqueue_script( 'Atozsites-osm', Atozsites_JS_URL . 'osm.js', array( 'jquery', 'jquery-ui-autocomplete', 'leaflet' ), Atozsites_VER, true );
	...
	public static function html( $meta, $field ) {
		...
		$html    = sprintf(
			'<div class="Atozsites-osm-field" data-address-field="%s">',
			esc_attr( $address )
		);

		$html .= sprintf(
			'<div class="Atozsites-osm-canvas" data-default-loc="%s" data-region="%s" data-language="%s"></div>
			<input type="hidden" name="%s" class="Atozsites-osm-coordinate" value="%s">',
			esc_attr( $field['std'] ),
	...
	public static function render_map( $location, $args = array() ) {
		...
		wp_enqueue_style( 'leaflet', 'https://unpkg.com/leaflet@1.5.1/dist/leaflet.css', array(), '1.5.1' );
		wp_enqueue_script( 'leaflet', 'https://unpkg.com/leaflet@1.5.1/dist/leaflet.js', array(), '1.5.1', true );
		wp_enqueue_script( 'Atozsites-osm-frontend', Atozsites_JS_URL . 'osm-frontend.js', array( 'jquery', 'leaflet' ), Atozsites_VER, true );
		...
		$output = sprintf(
			'<div class="Atozsites-osm-canvas" data-osm_options="%s" style="width:%s;height:%s"></div>',
