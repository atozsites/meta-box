 ...
 */
abstract class Atozsites_Input_Field extends Atozsites_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'Atozsites-input', Atozsites_CSS_URL . 'input.css', '', Atozsites_VER );
	}
	...
	public static function html( $meta, $field ) {
		...
		$wrapper_class = 'Atozsites-input-group';
		if ( version_compare( get_bloginfo( 'version' ), '5.3', '>=' ) ) {
			$wrapper_class .= ' Atozsites-input-dark';
		}
		...
		if ( $field['prepend'] ) {
			$output .= '<span class="Atozsites-input-group-prepend">' . esc_html( $field['prepend'] ) . '</span>';
		}
		...
		if ( $field['append'] ) {
			$output .= '<span class="Atozsites-input-group-append">' . esc_html( $field['append'] ) . '</span>';
		}
