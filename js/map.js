( function ( $, document, window, google, Atozsites, i18n ) {
	'use strict';
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.Atozsites-map-canvas' );
			this.canvas = this.$canvas[0];
			this.$coordinate = this.$container.find( '.Atozsites-map-coordinate' );
		...
		addListeners: function () {
			...
			$( window ).on( 'Atozsites_map_refresh', refresh );

			// Refresh on meta box hide and show
			Atozsites.$document.on( 'postbox-toggled', refresh );
		...
		autocomplete: function () {
			...
			if ( $( '.Atozsites-geo-binding' ).length ) {
				var geocodeAddress = that.geocodeAddress.bind( that );
		...
		findAddressField: function( fieldName ) {
			...
			$address = this.$container.closest( '.Atozsites-group-clone' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}

			// If map and address is inside a non-cloneable group.
			$address = this.$container.closest( '.Atozsites-group-wrapper' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}
	...
	function init( e ) {
		$( e.target ).find( '.Atozsites-map-field' ).each( createController );
	}

	function restart() {
		$( '.Atozsites-map-field' ).each( createController );
	}

	Atozsites.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Atozsites-input', restart );
} )( jQuery, document, window, google, Atozsites, Atozsites_Map );
