( function( $, L, Atozsites, i18n ) {
	...
	OsmField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.Atozsites-osm-canvas' );
			this.canvas = this.$canvas[0];
			this.$coordinate = this.$container.find( '.Atozsites-osm-coordinate' );
	...
	function init( e ) {
		$( e.target ).find( '.Atozsites-osm-field' ).each( createController );
	}

	function restart() {
		$( '.Atozsites-osm-field' ).each( createController );
	}

	Atozsites.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Atozsites-input', restart );
} )( jQuery, L, Atozsites, Atozsites_Osm );