( function ( $, wp, _, Atozsites, i18n ) {
	'use strict';

	var views = Atozsites.views = Atozsites.views || {},
		models = Atozsites.models = Atozsites.models || {},
		media = wp.media,
	...
	MediaCollection = Backbone.Collection.extend( {
		...
		remove: function ( models, options ) {
			...
			if( ! $( event.target ).closest( '.Atozsites-field, [data-class="Atozsites-field"]' ).length ) {
				return;
			}
	...
	MediaField = views.MediaField = Backbone.View.extend( {
		className: 'Atozsites-media-view',
		initialize: function ( options ) {
			...
			this.controller.get( 'items' ).on( 'add remove reset', _.debounce( function () {
				that.$input.trigger( 'change', [that.$( '.Atozsites-media-input' )] );
			}, 500 ) );
	...
	MediaList = views.MediaList = Backbone.View.extend( {
		tagName: 'ul',
		className: 'Atozsites-media-list',
	...
	MediaStatus = views.MediaStatus = Backbone.View.extend( {
		tagName: 'div',
		className: 'Atozsites-media-status',
		template: wp.template( 'Atozsites-media-status' ),
	...
	MediaButton = views.MediaButton = Backbone.View.extend( {
		tagName: 'div',
		className: 'Atozsites-media-add',
		template: wp.template( 'Atozsites-media-button' ),
		events: {
	...
	MediaItem = views.MediaItem = Backbone.View.extend( {
		tagName: 'li',
		className: 'Atozsites-media-item attachment',
		template: wp.template( 'Atozsites-media-item' ),
		initialize: function ( options ) {
			this.controller = options.controller;
			this.collection = this.controller.get( 'items' );
			this.render();
		...
		events: {
			'click .Atozsites-image-overlay': function ( e ) {
				e.preventDefault();
				this.trigger( 'click:switch', this.model );
			},
			'click .Atozsites-remove-media': function ( e ) {
				e.preventDefault();
				this.trigger( 'click:remove', this.model );
			},
			'click .Atozsites-edit-media': function ( e ) {
				e.preventDefault();
	...
	MediaSelect = views.MediaSelect = MediaFrame.Select.extend( {
		...
		createStates: function () {
			...
			this.$el.attr( 'data-class', 'Atozsites-field' );
	...
	function init( e ) {
		$( e.target ).find( '.Atozsites-file_advanced' ).each( initMediaField );
	}

	Atozsites.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Atozsites-file_advanced', initMediaField );
} )( jQuery, wp, _, Atozsites, i18nAtozsitesMedia );
