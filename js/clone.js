( function ( $, Atozsites ) {
	...
	var cloneIndex = {
		/**
		 * Set index for fields in a .Atozsites-clone
		 * @param $inputs .Atozsites-clone element
		...
		set: function ( $inputs, index ) {
			$inputs.each( function () {
				...
				// Name attribute
				var name = this.name;
				if ( name && ! $field.closest( '.Atozsites-group-clone' ).length ) {
		 ...
		 * @param $container .Atozsites-input container
		 * @return integer
		 */
	...
	var cloneValue = {
		setDefault: function() {
			...
			} else if ( $field.hasClass( 'Atozsites-checkbox' ) ) {
					$field.prop( 'checked', !! defaultValue );
			} else if ( $field.hasClass( 'Atozsites-checkbox_list' ) ) {
				var value = $field.val();
			...
			} else if ( ! $field.hasClass( 'Atozsites-hidden' ) ) {
	...
	function clone( $container ) {
		var $last = $container.children( '.Atozsites-clone' ).last(),
			$clone = $last.clone(),
			nextIndex = cloneIndex.nextIndex( $container );

		// Clear fields' values.
		var $inputs = $clone.find( Atozsites.inputSelectors );
	...
	function toggleRemoveButtons( $container ) {
		var $clones = $container.children( '.Atozsites-clone' );
		$clones.children( '.remove-clone' ).toggle( $clones.length > 1 );

		// Recursive for nested groups.
		$container.find( '.Atozsites-input' ).each( function () {
	...
	function toggleAddButton( $container ) {
		var $button = $container.children( '.add-clone' ),
			...
			numClone = $container.children( '.Atozsites-clone' ).length;
	...
	function addClone( e ) {
		...
		var $container = $( this ).closest( '.Atozsites-input' );
		clone( $container );
	...
	function removeClone( e ) {
		...
		var $this = $( this ),
			$container = $this.closest( '.Atozsites-input' );

		// Remove clone only if there are 2 or more of them
		if ( $container.children( '.Atozsites-clone' ).length < 2 ) {
			return;
		}
		...
		// Trigger custom change event for MB Blocks to update block attributes.
		$container.find( Atozsites.inputSelectors ).first().trigger( 'mb_change' );
	}

	/**
	 * Sort clones.
	 * Expect this = .Atozsites-input element.
	 */
	function sortClones() {
		var $container = $( this );
		...
		if ( 0 === $container.children( '.Atozsites-clone' ).length ) {
			return;
		}

		$container.sortable( {
			handle: '.Atozsites-clone-icon',
			placeholder: ' Atozsites-clone Atozsites-sortable-placeholder',
			items: '> .Atozsites-clone',
			start: function ( event, ui ) {
				...
				ui.item.find( '.Atozsites-wysiwyg' ).each( function () {
	...
	function start() {
		...
		$container.data( 'next-index', $container.children( '.Atozsites-clone' ).length );
		sortClones.apply( this );
	}

	function init( e ) {
		$( e.target ).find( '.Atozsites-input' ).each( start );
	}
