( function ( $, Atozsites ) {
	...
	function transform() {
		...
		if ( options.ajax_data ) {
			...
			options.ajax.transport = function ( params, success, failure ) {
				...
				var actions = {
					'post'             : 'Atozsites_get_posts',
					'taxonomy'         : 'Atozsites_get_terms',
					'taxonomy_advanced': 'Atozsites_get_terms',
					'user'             : 'Atozsites_get_users'
				};
	...
	function init( e ) {
		$( e.target ).find( '.Atozsites-select_advanced' ).each( transform );
	}

	Atozsites.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Atozsites-select_advanced', transform );
} )( jQuery, Atozsites );
