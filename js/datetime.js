( function ( $, _, Atozsites, i18n ) {
	...
	function transform() {
		var $this = $( this ),
			...
			$inline = $this.siblings( '.Atozsites-datetime-inline' ),
			$timestamp = $this.siblings( '.Atozsites-datetime-timestamp' ),
			current = $this.val(),
	...
	function init( e ) {
		$( e.target ).find( '.Atozsites-datetime' ).each( transform );
	}

	setTimeI18n();
	Atozsites.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Atozsites-datetime', transform );
} )( jQuery, _, Atozsites, Atozsites_Datetime );
