( function ( $, wp, window, Atozsites ) {
	...
	function getOriginalId( $el ) {
		var $clone = $el.closest( '.Atozsites-clone' ),
			currentId = $clone.find( '.Atozsites-wysiwyg' ).attr( 'id' );
	...
	function init( e ) {
		$( e.target ).find( '.Atozsites-wysiwyg' ).each( transform );
	}
	...
	Atozsites.$document
		...
		.on( 'clone', '.Atozsites-wysiwyg', function() {
			/*
